...
- **Compute line profiles**: Next, `pyROX` computes the Voigt profiles as the real part of the Faddeeva function [Eq. 12 of @Gandhi_ea_2020]. Although faster approximations exist [@Schreier_2018], we currently support only the more accurate `scipy.special.wofz` implementation[^3]. On a single CPU, this computes 10000 lines in about 3 seconds, using $\Delta\nu=0.01\ \mathrm{cm^{-1}}$ and a wing-cutoff of $100\ \mathrm{cm^{-1}}$.
