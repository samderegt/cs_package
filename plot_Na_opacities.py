...
for P in pressures:
    ...
    for i, T in enumerate(temperatures):
        ...
        wave_micron, opa1 = atm1.plot_opas(
            ['Na_lor_cut'], temperature=T, pressure_bar=P, return_opacities=True
            )['Na_lor_cut']
        wave_micron, opa2 = atm2.plot_opas(
            ['Na_wo_asymmetry'], temperature=T, pressure_bar=P, return_opacities=True
            )['Na_wo_asymmetry']
...
for T in temperatures:
    ...
    plt.tight_layout()
    plt.savefig('./Na_opacities_tot_T{:.0f}.pdf'.format(T))
    #plt.show()
    plt.close()